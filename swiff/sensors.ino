...
void readAnalogSensors() {
  ...
  // Koppel inlezen [Nm]
  Tas = analogRead(A2)/1023.0*referenceVoltage;
  Tas = analogRead(A2)/1023.0*referenceVoltage;
  ...
  Vbat = analogRead(A0)/1023.0*referenceVoltage*1080.8/90.8;  // [V]
  Vbat = analogRead(A0)/1023.0*referenceVoltage*1080.8/90.8;  // [V]
...
void readAnalogSensors2(int index) {
  ...
  Tas_array[index] = 0.109*analogRead(A2)/1023.0*referenceVoltage*1000 - 70; //0.006V per Nm
  Tas_sum += Tas_array[index];
  ...
  Vbat_array[index] = analogRead(A0)/1023.0*referenceVoltage*1080.8/90.8;  // [V]
  Vbat_sum += Vbat_array[index]; 
}
